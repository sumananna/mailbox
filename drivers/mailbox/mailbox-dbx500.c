...
static irqreturn_t dbx500_mbox_irq_handler(int irq, void *data)
{
	...
	for (n = 0; bits; n++) {
		if (bits & MBOX_BIT(n)) {
			bits -= MBOX_BIT(n);
			generic_handle_irq(irq_find_mapping(dbx500_mbox_irq_domain, n));
		}
	}
	return IRQ_HANDLED;
}
...
static int dbx500_mbox_probe(struct platform_device *pdev)
{
	...
	tcdm_mem_base = devm_ioremap(&pdev->dev, mem->start, resource_size(mem));
	if (!tcdm_mem_base) {
		ret = -ENOMEM;
		goto out;
	}
	...
	for (i = 0; list[i]; i++) {
		...
		if (!priv)
			continue;
		mbox->irq = irq_create_mapping(dbx500_mbox_irq_domain, mbox->id);
